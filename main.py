...
if __name__ == "__main__":
    ...
    with open("report.txt", "w") as file:
        file.write("RESPONSE TIME REPORT TO GET THE LEADERBOARD BY COUNTRY WITH DIFFERENT NUMBER OF RECORDS AND USER PLACE\n\n")

        # Case 1: Get leaderboard per country when the user is within the first 10 places
        file.write("Case 1: Get leaderboard per country when the user is within the first 10 places with 10K records\n")
        tic = time.perf_counter()
        leaderboard_of_case_1_with_10k = get_leaderboard_by_country(CASE_1_WITH_10K_RECORDS)
        ...
        file.write("Case 1: Get leaderboard per country when the user is within the first 10 places with 100K records\n")
        tic = time.perf_counter()
        leaderboard_of_case_1_with_100k = get_leaderboard_by_country(CASE_1_WITH_100K_RECORDS)
        toc = time.perf_counter()
        file.write(ELAPSED_TIME_MSG.format(elapsed_time=toc-tic))
        file.write(f"input: {CASE_1_WITH_100K_RECORDS}\n")
        file.write(f"output: {leaderboard_of_case_1_with_100k}\n\n")

        file.write("Case 1: Get leaderboard per country when the user is within the first 10 places with 1M records\n")
        tic = time.perf_counter()
        leaderboard_of_case_1_with_1m = get_leaderboard_by_country(CASE_1_WITH_1M_RECORDS)
        toc = time.perf_counter()
        file.write(ELAPSED_TIME_MSG.format(elapsed_time=toc-tic))
        ...
        file.write("Case 2: Get leaderboard by country when the user is between 11th place and 99th place with 10K records\n")
        tic = time.perf_counter()
        leaderboard_of_case_2_with_10k = get_leaderboard_by_country(CASE_2_WITH_10K_RECORDS)
        toc = time.perf_counter()
        file.write(ELAPSED_TIME_MSG.format(elapsed_time=toc-tic))
        file.write(f"input: {CASE_2_WITH_10K_RECORDS}\n")
        file.write(f"output: {leaderboard_of_case_2_with_10k}\n\n")

        file.write("Case 2: Get leaderboard by country when the user is between 11th place and 99th place with 100K records\n")
        tic = time.perf_counter()
        leaderboard_of_case_2_with_100k = get_leaderboard_by_country(CASE_2_WITH_100K_RECORDS)
        toc = time.perf_counter()
        file.write(ELAPSED_TIME_MSG.format(elapsed_time=toc-tic))
        file.write(f"input: {CASE_2_WITH_100K_RECORDS}\n")
        file.write(f"output: {leaderboard_of_case_2_with_100k}\n\n")

        file.write("Case 2: Get leaderboard by country when the user is between 11th place and 99th place with 1M records\n")
        tic = time.perf_counter()
        leaderboard_of_case_2_with_1m = get_leaderboard_by_country(CASE_2_WITH_1M_RECORDS)
        toc = time.perf_counter()
        file.write(ELAPSED_TIME_MSG.format(elapsed_time=toc-tic))
        ...
        file.write("Case 3: Get leaderboard by country when the user is above 99th place with 10K records\n")
        tic = time.perf_counter()
        leaderboard_of_case_3_with_10k = get_leaderboard_by_country(CASE_3_WITH_10K_RECORDS)
        toc = time.perf_counter()
        file.write(ELAPSED_TIME_MSG.format(elapsed_time=toc-tic))
        file.write(f"input: {CASE_3_WITH_10K_RECORDS}\n")
        file.write(f"output: {leaderboard_of_case_3_with_10k}\n\n")

        file.write("Case 3: Get leaderboard by country when the user is above 99th place with 100K records\n")
        tic = time.perf_counter()
        leaderboard_of_case_3_with_100k = get_leaderboard_by_country(CASE_3_WITH_100K_RECORDS)
        toc = time.perf_counter()
        file.write(ELAPSED_TIME_MSG.format(elapsed_time=toc-tic))
        file.write(f"input: {CASE_3_WITH_100K_RECORDS}\n")
        file.write(f"output: {leaderboard_of_case_3_with_100k}\n\n")

        file.write("Case 3: Get leaderboard by country when the user is above 99th place with 1M records\n")
        tic = time.perf_counter()
        leaderboard_of_case_3_with_1m = get_leaderboard_by_country(CASE_3_WITH_1M_RECORDS)
        toc = time.perf_counter()
        file.write(ELAPSED_TIME_MSG.format(elapsed_time=toc-tic))
